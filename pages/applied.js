                    ...
                    {allItems.map((item, index) => {
                        if (item.provider.toLowerCase() == walletAddress.toLowerCase()) {
                            ...
                            >
                                ...
                                {
                                    walletAddress.toLowerCase() == item.owner.toLowerCase() ? (<Text fontSize={"sm"} color={"gray.400"} mt={2}>You are the owner of this escrow.</Text>) : (<>
                                        {item.status < 2 && <Button mt={10} onClick={() => deliver(item.itemId)}>Confirm</Button>}
                                        <Text fontSize={"sm"} color={"gray.400"} mt={2}>Escrowed by {item.owner.substring(0, 6) + "..." + item.owner.slice(-4)}</Text></>)
                                }
                            </Flex>
                        }
                    })}
